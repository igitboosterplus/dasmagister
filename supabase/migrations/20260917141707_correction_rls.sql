...
-- Deuxième ancienne version clock_out
DROP FUNCTION IF EXISTS public.clock_out(
    double precision,
    double precision,
    double precision,
...
-- Ancienne version clock_in ayant la signature utilisée actuellement
...
BEGIN
    ...
    -- ========================================================
    -- 2.4 Le site doit appartenir à la structure de l'employé
    -- ========================================================
    ...
    -- ========================================================
    -- 2.5 Vérification employee_sites
    -- ========================================================
    ...
    -- ========================================================
    -- 2.6 Date/heure de l'événement
    -- ========================================================

    v_occurred_at := COALESCE(p_occurred_at, v_server_now);

    -- La date métier est calculée selon le fuseau du site
    v_attendance_date :=
        (v_occurred_at AT TIME ZONE COALESCE(v_site.timezone, 'Africa/Douala'))::date;


    -- ========================================================
    -- 2.7 Protection contre doublon client_event_id
    -- ========================================================
    ...
    IF p_client_event_id IS NOT NULL THEN
        ...
        WHERE ae.client_event_id = p_client_event_id
          AND ae.event_type::text = 'check_in'
       ...
       AND p_longitude IS NOT NULL THEN
        ...
        IF v_site.max_gps_accuracy_m IS NOT NULL
           AND p_accuracy_m > v_site.max_gps_accuracy_m THEN
            ...
            v_validation_status := 'low_accuracy';
        ...
        -- Vérification zone
        IF v_site.latitude IS NOT NULL
           AND v_site.longitude IS NOT NULL
           AND v_site.location_radius_m IS NOT NULL
           AND v_distance_m > v_site.location_radius_m THEN

            v_validation_status := 'out_of_zone';
    ...
    -- ========================================================
    -- 2.9 Vérifier qu'il n'existe pas déjà un pointage ouvert
    -- ========================================================
    ...
    v_day_of_week :=
        EXTRACT(
            DOW FROM (
                v_occurred_at
                AT TIME ZONE COALESCE(v_site.timezone, 'Africa/Douala')
            )
        )::integer;
    ...
    VALUES (
        ...
        p_occurred_at,
        CASE
            WHEN p_device_recorded_at IS NOT NULL
            THEN v_server_now
            ELSE NULL
        END,
        ...
        CASE
            WHEN FOUND THEN v_schedule.work_start
            ELSE NULL
        END,

        CASE
            WHEN FOUND THEN v_schedule.work_end
            ELSE NULL
        END,
    ...
    VALUES (
        ...
        'check_in'::attendance_event_type,
        v_check_in_method,
...
BEGIN
    ...
    IF v_employee.structure_id <> v_site.structure_id THEN
        RAISE EXCEPTION 'SITE_NOT_IN_EMPLOYEE_STRUCTURE'
            USING ERRCODE = 'P0001';
    END IF;
    ...
    -- ========================================================
    -- 3.5 Affectation employee_sites
    -- ========================================================
    ...
    IF p_client_event_id IS NOT NULL THEN
        ...
        JOIN public.attendance_events ae
          ON ae.attendance_id = a.id
        WHERE ae.client_event_id = p_client_event_id
          AND ae.event_type::text = 'check_out'
          AND a.employee_id = v_employee_id
        LIMIT 1;
    ...
    ELSE
        ...
        v_validation_method := 'manual';
        v_check_out_method := 'manual';

    END IF;


    -- ========================================================
    -- 3.9 Récupération du pointage ouvert
    -- ========================================================
    ...
    -- ========================================================
    -- 3.10 Vérification date
    -- ========================================================

    IF v_attendance.attendance_date > v_attendance_date THEN

        RAISE EXCEPTION 'INVALID_ATTENDANCE_DATE'
            USING ERRCODE = 'P0001';
    ...
    SET
        ...
        client_event_id = COALESCE(
            client_event_id,
            p_client_event_id
        ),

        device_recorded_at = COALESCE(
            device_recorded_at,
            p_device_recorded_at
        ),
    ...
    -- ========================================================
    -- 3.12 Création événement check_out
    -- ========================================================
    ...
    VALUES (
        ...
        'check_out'::attendance_event_type,
        v_check_out_method,
